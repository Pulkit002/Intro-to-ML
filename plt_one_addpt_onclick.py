...
class plt_one_addpt_onclick:
    ...
    def __init__(self, x, y, w, b, logistic=True):
        ...
        ax.scatter(x[pos], y[pos], marker='x', s=80, c = 'red', label="Expensive")
        ax.scatter(x[neg], y[neg], marker='o', s=100, label="inexpensive", facecolors='none', edgecolors=dlblue,lw=3)
        ax.set_ylim(-0.05,1.1)
        xlim = ax.get_xlim()
        ax.set_xlim(xlim[0],xlim[1]*2)
        ax.set_ylabel('Probability')
        ax.set_xlabel('Area(1k sq. feet)')
        ...
        ax.text(0.65,0.8,"Click anywhere to add points", size=10, transform=ax.transAxes)
...
#   @output.capture()  # debug
    ...
    def draw_thresh(self):
        ...
        c = self.ax[0].annotate("Expensive", xy= [xp5,0.5], xycoords='data',
        ...
        e = self.ax[0].annotate("Inexpensive", xy= [xp5,0.5], xycoords='data',
